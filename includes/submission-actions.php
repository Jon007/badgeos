...
function badgeos_save_nomination_data() {
	...
	absint( $_POST[ 'achievement_id' ] ), sprintf( '%1$s: %2$s', get_post_type( absint( $_POST[ 'achievement_id' ] ) ), get_the_title( absint( $_POST[ 'achievement_id' ] ) ) ), esc_textarea( $_POST[ 'badgeos_nomination_content' ] ), absint( $_POST[ 'badgeos_nomination_user_id' ] ), absint( absint( $_POST[ 'user_id' ] ) )
	);
}
...
function badgeos_process_submission_review( $post_id = 0 ) {
	...
	if (
	! empty( $_POST ) && isset( $_POST[ 'post_type' ] ) // verify post_type is set
...
function badgeos_save_submission_data() {
	...
	absint( $_POST[ 'achievement_id' ] ), sprintf( '%1$s: %2$s', get_post_type( absint( $_POST[ 'achievement_id' ] ) ), get_the_title( absint( $_POST[ 'achievement_id' ] ) ) ), esc_textarea( $_POST[ 'badgeos_submission_content' ] ), absint( $_POST[ 'user_id' ] ), $action
	);
}
...
function badgeos_set_submission_status_submission_pending( $messages, $args ) {
	...
	if ( $args[ 'badgeos_settings' ] && 'disabled' != $args[ 'badgeos_settings' ][ 'submission_email' ] ) {
		...
		__( 'A new submission has been received:
			...
			To view all submissions, visit: %4$s', 'badgeos' ), get_the_title( $args[ 'achievement_id' ] ), $args[ 'user_data' ]->display_name, admin_url( sprintf( 'post.php?post=%d&action=edit', $args[ 'submission_id' ] ) ), admin_url( 'edit.php?post_type=submission' )
		);
...
function badgeos_set_submission_status_nomination_pending( $messages, $args ) {
	...
	if ( $args[ 'badgeos_settings' ] && 'disabled' != $args[ 'badgeos_settings' ][ 'submission_email' ] ) {
		...
		__( 'A new nomination has been received:
			...
			To view all nominations, visit: %5$s', 'badgeos' ), get_the_title( $args[ 'achievement_id' ] ), $args[ 'user_data' ]->display_name, $args[ 'from_user_data' ]->display_name, admin_url( sprintf( 'post.php?post=%d&action=edit', $args[ 'submission_id' ] ) ), admin_url( 'edit.php?post_type=nomination' )
		);
...
function render_attachment_from_draft_submission( $attachment = array() ) {
	...
	if ( ! empty( $attachment ) ) {
		...
		$sub_form	 .= sprintf( __( '%1$s - uploaded %2$s by %3$s', 'badgeos' ), wp_get_attachment_link( $attachment->ID, 'full', false, null, $attachment->post_title ), get_the_time( 'F j, Y g:i a', $attachment ), $display_name
		);
	}
